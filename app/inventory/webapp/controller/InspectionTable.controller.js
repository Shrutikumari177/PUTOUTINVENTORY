    ...
    function (Controller, Fragment, JSONModel, MessageBox, MessageToast) {
        ...
        return Controller.extend("inventory.controller.InspectionTable", {
            ...
            _submitData: async function(updatedMaterials) {
                ...
                const materialReq = JSON.parse(sessionStorage.getItem("materialData"));
                const servNo = materialReq ? materialReq : null;
            ...
            quantityChange: function (oEvent) {
                ...
                let originalValue = parseFloat(oInput.data("originalValue")); // Retrieve the original value stored and convert it to a number
                 //Check if the originalValue exists, else set it to newValue initially
                if (isNaN(originalValue)) {
                    oInput.data("originalValue", newValue);
